...
const weatherData = data.map((d, idx) => {
...
export default function PowerChart() {
  ...
  const [range, setRange] = useState({ startIndex: 0, endIndex: hours - 1 });
  ...
  const resetZoom = () => {
    setRange({ startIndex: 0, endIndex: hours - 1 });
  };

  const filtered = data.slice(range.startIndex, range.endIndex + 1);

  const startTime = data[range.startIndex].timestamp;
  const endTime = data[range.endIndex].timestamp;
    ...
    <Box sx={{ width: '100%', height: 400 }}>
      ...
      <Box display="flex" height={300}>
        <Box flexGrow={1}>
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={filtered} margin={{ top: 10, right: 20, bottom: 30, left: 20 }}>
              ...
              <XAxis
                ...
                ticks={Array.from({ length: 4 }, (_, i) => new Date(start + i * 24 * 3600 * 1000).getTime())}
              />
              <YAxis
          ...
          {range.startIndex !== 0 || range.endIndex !== hours - 1 ? (
            <Box textAlign="right">
