...
if __name__ == "__main__":
    import argparse
    import json
    parser = argparse.ArgumentParser(description="Centrifuge cascade 8760 load model")
    parser.add_argument("--params-json", type=str, default=None,
                        help="Path to JSON file with CentrifugeParams fields")
    parser.add_argument("--csv", type=str, default=None, help="Output CSV path")
    parser.add_argument("--xlsx", type=str, default=None, help="Output XLSX path")
    ...
    if args.params_json:
        with open(args.params_json, "r") as f:
            raw = json.load(f)
        # Filter unknown keys
        known = {k: v for k, v in raw.items() if k in asdict(CentrifugeParams()).keys()}
        p = CentrifugeParams(**known)
    else:
        p = CentrifugeParams()

    df, summary = build_centrifuge_load_curve(p)
    save_outputs(df, summary, csv_path=args.csv, xlsx_path=args.xlsx)
